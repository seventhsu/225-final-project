...
//using a similar structure as to from lab_dict
Graph::Graph(const string & filename) {
    ...
    if (wordsFile.is_open()) {
        while (getline(wordsFile, line)) {
            ...
            SplitString(line, '\t', splittedWords);
            if (splittedWords.size() != 2) {
                continue;
            }
            insertVertex(splittedWords[0]), insertVertex(splittedWords[1]), updateEdgeWeight(splittedWords[0], splittedWords[1], 1);
        }
    }
}
...
void Graph::insertVertex(Vertex v)
{
    ...
    //removeVertex(v);
    // if we remove, we lose what we previously added
    //instead, if we already have the vertex, we will just return
...
void Graph::updateEdgeWeight(Vertex source, Vertex destination, int weightIncrement) {
    if (assertEdgeExists(source, destination, __func__) == false) {
        //copied over from insert edge basically
        // if the edge did not exist, we insert a new one with weight 1
        adjacency_list[source][destination] = Edge(source, destination, 1, "");
        if (!directed)
        {
            if(adjacency_list.find(destination)== adjacency_list.end())
            {
            ...
            adjacency_list[destination][source] = Edge(source, destination, 1, "");
        }
    }
    Edge e = adjacency_list[source][destination];
    //std::cout << "setting weight: " << weight << std::endl;
    // technically, we don't really need a weightIncrement argument as we will always be incrementing by 1 anyway
    // however, for now, it might be good to keep just in
    // perhaps if we figure out some way to keep track of how many times the weight must be incremented in the constructor
    // we can then just use that as the argument instead of calling this function each and every time
    Edge new_edge(source, destination, e.getWeight() + weightIncrement, e.getLabel());
    adjacency_list[source][destination] = new_edge;

    if(!directed)
        {
            Edge new_edge_reverse(destination,source, e.getWeight() + weightIncrement, e.getLabel());
            adjacency_list[destination][source] = new_edge_reverse;
        }
